...
// - src/components/Pages/Blog/Blog.jsx (blog listing page)
// - src/components/Pages/Blog/ArticlePage.jsx (individual blog articles)
// - src/components/Pages/Recipe/RecipePage.jsx (individual recipe pages)
// - src/components/Pages/Historia/HistoriaOMnie.jsx ("About Me" history page)
// - src/components/Pages/Historia/HistoriaOAutyzmie.jsx ("About Autism" page)
// - src/components/Pages/Znajdki/ZnajdkiPage.jsx ("Finds" product listing page)
// - src/components/Pages/Znajdki/ZnajdkiProductPage.jsx (individual product pages)
// - src/components/Pages/Contact/ContactPage.jsx (contact page)
// - src/components/Pages/Search/SearchPage.jsx (search results page)
//
  ...
  const renderDropdownMenu = (items) => (
    ...
    >
      {items.map((item, index) => (
        <li
          key={index}
          className={`px-4 py-2 transform transition-all duration-300 ${
          ...
          style={{
            ...
            animation:
              activeDropdown && !isMobileMenuOpen
                ? `fadeInItem 0.4s ease-out forwards ${150 + index * 50}ms`
                : 'none',
          }}
        >
          <Link
            ...
            className={`text-base font-['Patrick_Hand'] font-medium uppercase tracking-wider leading-[1.2] whitespace-nowrap transition-all duration-300 mix-blend-normal block ${
              isMobileMenuOpen
                ...
                : 'text-gray-50/95 hover:text-yellow-400'
            }`}
          >
            {item.label}
          </Link>
          ...
          <div className="relative h-full flex items-center justify-center p-4 sm:p-6">
            <div className="bg-gray-900/95 backdrop-blur-md rounded-2xl shadow-2xl border border-gray-700/50 w-full max-w-sm max-h-[85vh] overflow-y-auto">
              <div className="flex items-center justify-between p-6 border-b border-gray-700/50">
                <h2 className="text-2xl font-['Patrick_Hand'] text-white font-bold">
                  Menu
                </h2>
                <button
                  onClick={() => setIsMobileMenuOpen(false)}
                      ...
                      <Link
                        ...
                        onClick={(e) => {
                          if (item.dropdown) {
                            e.preventDefault();
                            setActiveDropdown(
                              activeDropdown === item.type ? null : item.type,
                            );
                          } else {
