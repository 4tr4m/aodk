// HeroSection Component - Main hero section for the homepage
// This component displays the main hero banner with parallax background, animated text,
// and call-to-action button. It includes smooth scroll effects and interactive elements.
//
// USAGE LOCATIONS:
// - src/pages/HomePage.jsx (main landing page hero section - only used on homepage)
//
// The component is the primary visual element on the homepage and includes:
// - Parallax background image with scroll effects
// - Animated text that fades and moves on scroll
// - Interactive logo that opens an info modal
// - Call-to-action button that scrolls to categories section
...
const HeroSection = () => {
  ...
  // Framer Motion scroll hook for scroll-based animations
  const { scrollY } = useScroll();
  
  // Parallax effect for the background image - moves slower than scroll
  const backgroundY = useTransform(scrollY, SCROLL_RANGES.BACKGROUND, ANIMATION_VALUES.BACKGROUND_Y);
  
  // Text fade and move effects when scrolling down
  const textOpacity = useTransform(scrollY, SCROLL_RANGES.TEXT, ANIMATION_VALUES.TEXT_OPACITY); // Fades out
  const textY = useTransform(scrollY, SCROLL_RANGES.TEXT, ANIMATION_VALUES.TEXT_Y); // Moves up

  // Callback function to toggle the info modal popup
  ...
  return (
    <>
      ...
      <header className="min-h-screen flex flex-col">
        {/* Top section with logo and text content */}
        <div className="pt-8 pb-4 static text-gray-50 z-1 shadow-[0_2px_4px_rgba(0,0,0,0.3)]">
          <div className="max-w-7xl mx-auto px-3 sm:px-5 flex flex-col md:flex-row items-center md:items-start justify-between gap-3 sm:gap-5 static">
            ...
            {/* Hero text content - main headline and description */}
            <div className="flex-2 group relative z-10 mt-4 md:mt-0">
              <div className="max-w-3xl mx-auto py-4 sm:py-6 px-4 sm:px-8 md:px-16 relative">
                ...
                {/* Animated text container with scroll effects */}
                <motion.div 
            ...
            {/* Spacer div for desktop layout balance */}
            <div className="hidden md:block flex-[0.8]"></div>
          </div>
        </div>

        {/* Spacer section to push content to bottom */}
        <div className="flex justify-center items-end flex-grow">
          {/* Empty space above the transition */}
        </div>
        
        {/* Bottom transition section with gradient and call-to-action */}
        <div className="absolute bottom-0 left-0 w-full overflow-hidden">
          ...
          <div className="w-full h-48 sm:h-56 md:h-64 bg-gradient-to-b from-transparent via-[#2D3748]/60 to-[#2D3748]/95">
            {/* Integrated banner text - tagline */}
          ...
          {/* Smooth curved transition to next section */}
          <div className="absolute bottom-0 left-0 w-full h-10 sm:h-12 md:h-16 overflow-hidden">
            ...
            ></div>
            {/* Curved white section that connects to the next content area */}
            <div 
      ...
      {/* Info modal that opens when logo is clicked */}
